...
export const CursorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [variant, setVariant] = useState<CursorVariant>("glass");
...
export const Cursor = () => {
  ...
  return (
    <div
      ...
      aria-hidden="true"
      className={cn(
        "size-6 rounded-full fixed pointer-events-none",
        isActive && "size-5 mt-0.5 ml-0.5",
        state == 'text' && `w-1 h-8 mx-[0.7rem] -translate-y-[40%]`,
        state == 'pointer' && "border",
        variant == 'glass' && "opacity-20 backdrop-blur-sm",
        invert && "backdrop-invert opacity-20"
      )}
      style={{
